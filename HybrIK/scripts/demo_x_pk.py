...
for img_path in tqdm(img_path_list):
    ...
    with torch.no_grad():
        ...
        if not opt.cpu:
            ...
            det_input = det_transform(input_image).cpu()
        ...
        if not opt.cpu:
            ...
            pose_input = pose_input.cpu()[None, :, :, :]
    ...
    if opt.save_pk:
        ...
        pred_xyz = pose_output.pred_xyz_hybrik.reshape(-1,3).cpu().data.numpy()
        #print(pred_xyz.shape)
        #(71, 3)
        pred_xyz = pred_xyz[:23]
        #print(pred_xyz.shape)
        #(23, 3)
        ...
        res_db['pred_xyz'].append(pred_xyz)
