...
# sourcemotion = os.path.join(Basepath + 'res.pk')
# sourcemotion = os.path.join('/home/cvlab/Juni/GroundLink/res.pk')
...
if __name__ == "__main__":
    ...
    print(CoP.shape, CoP_pred.shape, GRF.shape, GRF_pred.shape)
    # CoP_pred = torch.zeros(GRF_pred.shape)
    # CoP_pred = CoP_pred[:925, :, :]
    # print(CoP.shape, CoP_pred.shape, GRF.shape, GRF_pred.shape)
    # CoP.shape, torch.Size([4030, 2, 3])
    # CoP_pred.shape, torch.Size([4030, 2, 3])
    # GRF.shape, torch.Size([4030, 2, 3])
    # GRF_pred.shape, torch.Size([4030, 2, 3])
    # file = './GRF/scripts/hyrbik'
    #
    # GRF_pred = torch.load(file)
    print("CoP", CoP.numpy())
    print("GRF", GRF.numpy())
    print("Added", CoP.numpy()+GRF.numpy())

    print("my CoP", CoP_pred.numpy())
    print("my GRF", GRF_pred.numpy())
    print("Added", CoP_pred.numpy()+GRF_pred.numpy())
