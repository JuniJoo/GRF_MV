...
def rotate_vis(data):
    return torch.matmul(torch.tensor(Rx(-0.5 * m.pi)), data.transpose(-1, -2).type('torch.DoubleTensor')).transpose(-1,
                                                                                                                    -2)
