...
class Forces:
	...
	def gather(cls, forces):														# [...] x F x LR x 16
		...
		cell_groups = cls.FRONT_CELLS + cls.BACK_CELLS
		# print(cell_groups) # [[8, 9, 10, 11, 12, 13, 14, 15], [0, 1, 2, 3] ]
		# group_forces = [forces[..., cells].sum(dim=-1) for cells in cell_groups]	# [...] x F x LR
		group_forces = []
		for cells in cell_groups:
			print("cells", cells)
			print("forces", forces.shape)
			print(forces[..., cells].shape)
			group_sum = forces[..., cells].sum(dim=-1)  # Sum forces over the last dimension
			print('a', group_sum.shape) #torch.Size([1, 5836, 2]
			group_forces.append(group_sum)
		ppp
...
class Contacts:
	...
	def from_forces(cls, forces):													# [...] x F x LR x 16
		...
		# print(forces.shape) #torch.Size([1, 5836, 2, 6])

		# Compute raw contacts
		foot_forces = forces.sum(dim=-1).unsqueeze(-2)								# [...] x F x  1 x LR
		# print('a', foot_forces.shape) #torch.Size([1, 5836, 1, 2]
		total_force = foot_forces.sum(dim=-1, keepdim=True)							# [...] x F x  1 x  1
		# print('b', total_force.shape) #torch.Size([1, 5836, 1, 1]
